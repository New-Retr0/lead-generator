-- AI Gateway sales-copy generation (why_call / talking_points) is removed from the
-- pipeline. Drop and recreate sales_leads + partner_leads_v1 without those columns;
-- historical enriched_json.why_this_is_a_good_fit / sales_talking_points values are
-- left untouched on public.leads, just no longer projected through these views.

drop view if exists public.sales_leads;
