...
def test_python_verified_dm_contract(case: dict[str, Any]) -> None:
    ...
    expected_status = "Verified" if case["expected"] else "Unverified"
    assert lead.sales_status() == expected_status


@pytest.mark.parametrize("case", CASES, ids=[case["name"] for case in CASES])
def test_verification_level_aligns_with_verified(case: dict[str, Any]) -> None:
    """Computed level matches Verified: atomic DM → verified; Verified requires that level."""
    ...
    expected_status = "Verified" if atomic else "Unverified"
    assert lead.sales_status() == expected_status
...
def test_verified_view_uses_verified_dm_and_score_gate() -> None:
